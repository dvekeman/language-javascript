...
instance Pretty JSExpression where
    ...
    pretty (JSHexInteger     annot i) = text $ show i
    pretty (JSOctal          annot i) = text $ show i
...
--     -- Non-Terminals
    ...
    pretty (JSCommaExpression      le c re)                = pretty le <> text "," <> pretty re
    ...
    pretty (JSFunctionExpression   annot n lb x2s rb x3)   = text "function" <+> pretty n <> parens (pretty x2s) <+> prettyNestedBracesBlock x3
...
instance Pretty JSTryCatch where
    pretty (JSCatch anc alb x1 arb x3) = hsep [text "catch", parens (pretty x1)] <+> prettyNestedBracesBlock x3
    pretty (JSCatchIf anc alb x1 aif ex arb x3) = hsep [text "catch", parens (pretty x1 <+> text "if" <+> pretty ex)] <+> prettyNestedBracesBlock x3
...
instance Pretty JSStatement where
    pretty (JSStatementBlock alb blk arb s)             = (nest 2 (vcat $ map pretty blk)) <> pretty s
    ...
    pretty (JSFor af alb x1s s1 x2s s2 x3s arb x4)      = text "for" <> parens (hsep [pretty x1s <> text ";" , pretty x2s <> text ";" , pretty x3s]) <+> prettyNestedBracesBlock x4
    pretty (JSForIn af alb x1s i x2 arb x3)             = text "for" <> parens (pretty x1s <+> pretty i <+> pretty x2) <+> prettyNestedBracesBlock x3
    ...
    pretty (JSSwitch annot alp x arp alb x2 arb s)      = text "switch" <+> parens (pretty x) <+> prettyNestedBracesBlock (vcat (map pretty x2))
    pretty (JSThrow annot x s)                          = text "throw" <+> pretty x <> pretty s
    pretty (JSTry annot tb tcs tf)                      = text "try" <+> prettyNestedBracesBlock tb <$$>
																														      vcat (map pretty tcs ++ (pretty tf):[])
    pretty (JSVariable annot xs s)                      = text "var" <+> pretty xs <> pretty s
    pretty (JSWhile annot alp x1 arp x2)                = text "while" <+> parens (pretty x1) <+> prettyNestedBracesBlock x2
    pretty (JSWith annot alp x1 arp x s)                = text "with" <+> parens (pretty x1) <+> prettyNestedBracesBlock x <> pretty s
...
instance Pretty JSBlock where
    pretty (JSBlock alb ss arb) = vcat $ map pretty ss
-- 
