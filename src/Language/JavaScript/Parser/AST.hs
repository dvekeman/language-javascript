...
showStripped :: JSAST -> JSAST
showStripped (JSAstProgram xs _) = JSAstProgram (map ss xs) JSNoAnnot
...
instance ShowStripped JSStatement where
    ss (JSStatementBlock _ xs _ s) = JSStatementBlock JSNoAnnot (map ss xs) JSNoAnnot (ss s)
    ss (JSBreak _ x s) = JSBreak JSNoAnnot (ss x) (ss s)
    ss (JSContinue _ x s) = JSContinue JSNoAnnot (ss x) (ss s)
    ss (JSConstant _ xs s) = JSConstant JSNoAnnot (ss xs) (ss s)
    ...
    ss (JSForVarIn _ _ _ x1 op x2 _ x3) = JSForVarIn JSNoAnnot JSNoAnnot JSNoAnnot (ss x1) (ss op) (ss x2) JSNoAnnot (ss x3)
    ...
    ss (JSSwitch _ _ x _ _ x2 _ s) = JSSwitch JSNoAnnot JSNoAnnot (ss x) JSNoAnnot JSNoAnnot (map ss x2) JSNoAnnot (ss s)
    ss (JSThrow _ x s) = JSThrow JSNoAnnot (ss x) (ss s)
    ss (JSTry _ xt1 xtc xtf) = JSTry JSNoAnnot (ss xt1) (map ss xtc) (ss xtf)
    ss (JSVariable _ xs s) = JSVariable JSNoAnnot (ss xs) (ss s)
    ss (JSWhile _ _ x1 _ x2) = JSWhile JSNoAnnot JSNoAnnot (ss x1) JSNoAnnot (ss x2)
    ss (JSWith _ _ x1 _ x s) = JSWith JSNoAnnot JSNoAnnot (ss x1) JSNoAnnot (ss x) (ss s)

instance ShowStripped JSExpression where
    ss (JSArrayLiteral _lb xs _rb) = JSArrayLiteral JSNoAnnot (map ss xs) JSNoAnnot
    ...
    ss (JSDecimal _ s) = JSDecimal JSNoAnnot s
    ...
    ss (JSHexInteger _ s) = JSHexInteger JSNoAnnot s
    ss (JSOctal _ s) = JSOctal JSNoAnnot s
    ss (JSIdentifier _ s) = JSIdentifier JSNoAnnot s
    ss (JSLiteral _ s) = JSLiteral JSNoAnnot s
    ...
    ss (JSRegEx _ s) = JSRegEx JSNoAnnot s
    ss (JSStringLiteral _ s) = JSStringLiteral JSNoAnnot s
...
instance ShowStripped JSTryCatch where
    ...
    ss (JSCatchIf _ _ x1 _ ex _ x3) = JSCatchIf JSNoAnnot JSNoAnnot (ss x1) JSNoAnnot (ss ex) JSNoAnnot (ss x3)
...
instance ShowStripped JSIdent where
    ss (JSIdentName _ s) = JSIdentName JSNoAnnot s
    ss JSIdentNone = JSIdentNone

instance ShowStripped JSObjectProperty where
    ss (JSPropertyNameandValue x1 _colon x2s) = JSPropertyNameandValue (ss x1) JSNoAnnot (map ss x2s)
    ss (JSPropertyAccessor s x1 _lb1 x2s _rb1 x3) = JSPropertyAccessor (ss s) (ss x1) JSNoAnnot (map ss x2s) JSNoAnnot (ss x3)

instance ShowStripped JSPropertyName where
    ss (JSPropertyIdent _ s) = JSPropertyIdent JSNoAnnot s
    ss (JSPropertyString _ s) = JSPropertyString JSNoAnnot s
    ss (JSPropertyNumber _ s) = JSPropertyNumber JSNoAnnot s

instance ShowStripped JSAccessor where
    ss (JSAccessorGet _) = JSAccessorGet JSNoAnnot

instance ShowStripped JSBlock where
    ss (JSBlock _ xs _) = JSBlock JSNoAnnot (map ss xs) JSNoAnnot

instance ShowStripped JSSwitchParts where
    ss (JSCase _ x1 _c x2s) = JSCase JSNoAnnot (ss x1) JSNoAnnot (map ss x2s)
    ss (JSDefault _ _c xs) = JSDefault JSNoAnnot JSNoAnnot (map ss xs)
